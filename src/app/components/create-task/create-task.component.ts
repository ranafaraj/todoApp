...
if(formData){
  ...
  onSubmit(){
    ...
    if(formData && formData.key) {
      this.myForm.value.Colour = this.selectedColour;
      this.myForm.value.Icon = this.selectedIcon;
      this.todoService.addTodo(formData.key,this.myForm.value).then(data => {

        this.presentToast("Task Updated Succesfully!");
        this.cdr.detectChanges();
        this.modalCtrl.dismiss();
        this.modalCtrl.dismiss().then(()=>window.location.reload());
      })
    }else {
      this.currentDate = (new Date()).toISOString();
      let uid = this.currentDate + this.myForm.value.Title;
      this.myForm.value.Color = this.selectedColour;
      this.myForm.value.Icon = this.selectedIcon;
      this.todoService.addTodo(uid,this.myForm.value).then(data =>{
       console.log(data)
